---
name: speckit-tasks
description: "Python-shaped task generation. Converts a feature plan into a dependency-ordered task list — contracts first, then domain, repository, service, surface, wiring, and tests — with Python-idiomatic titles, acceptance criteria that reference constitution directives, and explicit Phase tags. Use when the user wants an ordered, checkable task list from a feature plan."
---

# Speckit Tasks

> This skill is generated from the Python preset command
> `presets/python/commands/speckit.tasks.md` by `scripts/build-skills.py`.
