...
description: "Deep codebase audit. Runs the regex-based audit script, then layers in ruff check, ruff format --check, mypy --strict, pytest, and pip-audit, plus file-level read-throughs and cross-file LLM analysis to confirm and correlate findings. Slower than /speckit.audit but materially higher signal. Use when the user wants a deep quality gate (ruff + mypy + pytest + pip-audit) before a release."
---

# Speckit Audit Deep

> This skill is generated from the Python preset command
> `presets/python/commands/speckit.audit.deep.md` by `scripts/build-skills.py`.
