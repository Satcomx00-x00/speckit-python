...
description: "Sync agent context files (AGENTS.md, CLAUDE.md, .github/copilot-instructions.md, GEMINI.md) from the installed agent-context.md template. Shows a diff for review before writing, keeping all agent context files in lockstep with the Python preset so every agent reads the same operating rules. Use when the user wants to sync AGENTS.md / CLAUDE.md / Copilot / Gemini context files."
---

# Speckit Docs Sync

> This skill is generated from the Python preset command
> `presets/python/commands/speckit.docs.sync.md` by `scripts/build-skills.py`.
...
- `--template <path>` — path to an alternate agent-context template (default: `.specify/presets/python/templates/agent-context.md`)

## Pre-Execution Checks

Check for `.specify/extensions.yml`. Look for hooks under `hooks.before_docs_sync`. Apply standard hook-processing (skip disabled, surface optional as instructions, auto-execute mandatory).

## Outline

Agent context files drift from the canonical template as the template evolves but the files are not updated. This command brings them back in sync, with a diff for review so no project-specific additions are lost silently.

### 1. Locate the template

Read the agent context template from:
1. `--template <path>` if provided
2. `.specify/presets/python/templates/agent-context.md` (installed preset location)
3. `presets/python/templates/agent-context.md` (monorepo development fallback)

If none of these paths exist, abort with a clear error message naming the paths checked.
...
**Template**: `.specify/presets/python/templates/agent-context.md`
**Targets checked**: <N>

| File | Status | Changes |
|---|---|---|
| `CLAUDE.md` | CONFLICT | +14 lines in template, -3 custom lines in current file |
| `AGENTS.md` | ADDITIVE | +14 lines in template, current file unchanged |
...
- Line 42: `<!-- Custom: this repo forbids async in lib/ -->`
- ...

#### AGENTS.md (ADDITIVE)

```diff
+ <new lines from the updated template, e.g. a new Concurrency & async rule>
```
...
- Do not add any machine-generated header (e.g. `<!-- auto-generated -->`); the template is the canonical source and should read like it was written by a human.
...
**Files written**:
- `CLAUDE.md` — overwritten (was CONFLICT; <N> custom lines lost — re-add them below or open a PR to fold them into the template)
...
- If the template itself needs updating, edit `.specify/presets/python/templates/agent-context.md` and re-run `/speckit-docs-sync`
```

### 8. Detect outdated constitution reference (advisory)

The agent context template asserts it is *derived from the constitution at `.specify/memory/constitution.md`*. After syncing, check whether that file exists. If it does not:

```
Advisory: No constitution found at `.specify/memory/constitution.md`.
Agent context files name it as their source of truth, but it has not been generated yet.
Run `/speckit-constitution-scan` to produce the constitution from the repository inventory
(pyproject.toml, tooling config, package layout).
```
...
then re-running `/speckit-docs-sync` to propagate any directive changes.
```
...
- **code quality** → `.specify/memory/knowledge/code-quality.md`
