<!--
SYNC IMPACT REPORT — generated by hand for the toolkit's own repository
====================================================================
Source template : presets/python/templates/constitution-template.md
Generated       : 2026-06-11
Operating phase : P1 — Foundation (this repo ships specs/templates, not a runtime app)
Version         : 1.0.0 (initial ratification)

Inventory snapshot (this repository):
  - pyproject.toml      : present — Ruff + mypy(strict) + pytest configured
  - uv.lock             : not yet committed (no runtime deps) — UNVERIFIED until `uv lock`
  - .python-version     : not pinned — TODO before adding runtime code
  - Ruff config         : present  ([tool.ruff] in pyproject.toml)            → MET
  - mypy --strict       : present  ([tool.mypy] strict = true)                → MET
  - pytest config       : present  ([tool.pytest.ini_options])                → MET
  - py.typed            : n/a (no installed package yet)                      → UNVERIFIED
  - ADRs                : docs/adr/0001 accepted                              → MET

Per-directive status is summarized above by section; directives that require a
running application (DB migrations, deploy artifacts, SLOs) are UNVERIFIED for
this spec-only repository and apply to projects that ADOPT this preset.

Follow-up TODOs:
  - Pin the interpreter (.python-version) and run `uv lock` once runtime code lands.
  - Re-run /speckit.constitution.scan in any project that adopts this preset to
    generate that project's own evidence-mapped constitution.

This file is the filled instance of the Python constitution template. The
behavior matrix below is the source of truth and is NOT rewritten by scans.
====================================================================
-->

# speckit-python Constitution
